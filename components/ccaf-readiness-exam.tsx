...
const BANK: Q[] = [
  ...
  },
  {
    ...
    opts: [
      ...
      "A fixed round-robin handoff chain regardless of which specialist is actually relevant",
    ],
    a: 2,
  ...
  },
  {
    ...
    opts: [
      "It violates per-role tool scoping - a subagent should only get the tools its role needs, even if more are declared project-wide",
  ...
  },
  {
    ...
    opts: [
      ...
      "Extracted as durable facts into external state, re-injected into each request rather than relied upon to survive in raw history",
    ],
    a: 3,
  ...
  },
  {
    ...
    opts: [
      ...
      "Tools (model-invoked), Resources (host-application-controlled context), Prompts (user-selected templates)",
  ...
  },
  {
    ...
    opts: [
      ...
      "A .claude/rules/ file with no frontmatter, relying on the wording alone to self-limit scope",
  ...
  },
  {
    d: "d3",
    s: "Structured Data Extraction",
    q: 'The extraction team wants a repeatable "/reprocess-batch $ARGUMENTS" shortcut for a routine manual task, and separately wants a capability that runs in an isolated context with only a narrow, fixed set of tools available while active. Which mechanisms fit each need, respectively?',
    opts: [
      "An Agent Skill for the shortcut; a slash command for the isolated, tool-restricted case",
      "A custom slash command (.claude/commands/*.md with $ARGUMENTS) for the shortcut; an Agent Skill with context: fork and disallowed-tools frontmatter for the isolated case",
  ...
  },
  {
    ...
    opts: [
      ...
      "By blast radius - the small, well-defined rename can proceed directly; the large, ambiguous restructuring should go through plan mode with approval first",
  ...
  },
  {
    ...
    opts: [
      ...
      "Scope the review prompt tightly to specific, named concerns (e.g. security regressions, breaking API changes) to minimize false positives",
    ],
    a: 3,
  ...
  },
  {
    ...
    opts: [
      ...
      "Replace the vague guidance with explicit, categorical criteria (a checklist each transcript must satisfy)",
  ...
  },
  {
    ...
    opts: [
      "Use tool_use with a JSON input_schema (or structured outputs) so the response is schema-validated, instead of asking for \"JSON\" in plain text",
  ...
  },
  {
    ...
    opts: [
      ...
      "A bounded retry loop - re-prompt with the validation error appended, up to a fixed retry count",
    ],
    a: 3,
  ...
  },
  {
    ...
    opts: [
      ...
      "A cheap draft pass followed by a stronger review pass that checks the draft against explicit criteria",
  ...
  },
  {
    ...
    opts: [
      ...
      "The Batches API has up to a 24-hour processing window and no real-time SLA - it's an anti-pattern for a synchronous user-facing flow; live replies belong on the real-time API",
  ...
  },
  {
    ...
    opts: [
      "Put the stable content (system prompt, tool definitions, the large document) first and mark it cacheable; put the volatile content (the latest question) last",
  ...
  },
  {
    ...
    opts: [
      ...
      "A structured state summary - what's been tried, what's confirmed, and what's still open - not the raw transcript",
    ],
    a: 3,
  ...
  },
  {
    ...
    opts: [
      ...
      "The partial result must be marked incomplete/degraded so the downstream refund-calculation subagent treats it accordingly, not as a normal complete result",
