...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const suffix =
    lab.kind === "soc"
      ? "Hands-On SOC Lab (SIEM & SOAR)"
      : isFree
      ? "Free Hands-On AWS Security Lab"
      : "Hands-On AWS Security Lab";
  ...
  const schema = [
    webPageSchema({
      ...
      breadcrumb: [
        ...
        { name: "Hands-on Labs", url: `${SITE.url}/labs-wizard` },
        { name: lab.title, url },
      ],
    }),
      ...
      <section className="relative isolate overflow-hidden border-b border-line">
        ...
        <Container className="py-10 sm:py-12">
          <Reveal className="max-w-3xl">
            <Link href="/labs-wizard?track=aws-security-labs" className="inline-flex items-center gap-2 text-sm font-semibold text-brand-bright">
              ← All hands-on labs
            </Link>
